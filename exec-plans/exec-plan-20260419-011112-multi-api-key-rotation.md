...
- [x] (2026-04-19T02:24:37-07:00) Ran Milestone 2 review round 2 against the full diff from `7988db6` to `ec4f0f313f77e050f8eb009b47db79ed4c05bdd0`. The only remaining finding was a minor regression-coverage gap around `LiveViewModel.StartAsync()` actually loading all stored keys before issuing the request.
...
- Observation: the most practical way to cover the real frontend start wiring without introducing a WinUI test harness was to move the “load all stored keys and build the start request” step into a Core helper that the existing test project can exercise directly.
  Evidence: Milestone 2 review round 2 identified that DTO-serialization tests alone would not catch a regression back to singular key loading inside `LiveViewModel.StartAsync()`.
...
- Decision: The `LiveViewModel` start path now delegates request preparation to a Core helper rather than loading and checking stored keys inline.
  Rationale: This gives the current test project direct regression coverage over the actual multi-key start preparation logic without taking a dependency on the WinUI app assembly.
  Date/Author: 2026-04-19 / Codex

- Decision: Runtime rotation is implemented in a new backend helper module rather than by rewriting the CLI-oriented code in `rolling_transcriber.py`.
  Rationale: The desktop app’s service/session path needs multi-key selection, but the CLI path is explicitly staying single-key for this iteration. Isolating the pool helper keeps that boundary clear and lets `test_stable_prefix.py` continue validating the untouched CLI surface.
  Date/Author: 2026-04-19 / Codex

- Decision: The client pool serializes request selection and execution under one lock, then advances the global cursor only after a successful attempt.
...
- Decision: Final local verification in this terminal session will treat the worktree desktop-app launch plus backend `healthz` probe as the practical smoke-test boundary; the richer Settings and Live-page walkthrough remains a manual follow-up outside the current non-interactive environment.
  Rationale: The repository has automated coverage for the storage, contract, and service semantics, and this environment can reliably verify process startup plus backend readiness, but it does not include a stable WinUI UI-automation harness for end-to-end button and text-entry interaction.
  Date/Author: 2026-04-19 / Codex

## Outcomes & Retrospective

Milestone 1 is implemented and passes its automated verification. After the third and final allowed review round, the Windows-side multi-key storage flow now covers the main recovery cases called out during review: legacy raw-string payloads, malformed JSON envelopes, decrypt failures, file-read failures, empty-editor saves, and the “reveal a bad payload then immediately replace it” path all now degrade to actionable UI guidance instead of hidden or crashy failure modes. In this terminal session, Milestone 1 was signed off on the strength of `WindowsSecretStore`-level automated coverage rather than a completed Settings-page click-through; that richer UI walkthrough now sits in the explicit follow-up manual QA section at the end of the plan.

Milestone 2 is implemented and passes its automated verification. Both the Windows frontend and backend service now speak `api_keys`, the backend explicitly rejects legacy `api_key` start requests and all secret fields in `/settings`, and the frontend start wiring now has direct regression coverage for loading all stored keys before request creation. The eventual desktop-app bootstrap smoke was covered in Milestone 4, but a richer Live-page Start interaction check still remains part of follow-up manual QA instead of being represented as completed evidence in this terminal session.

Milestone 3 is implemented and passes its automated verification. The backend no longer pins a session to one client object, but instead routes each transcription request through a deterministic multi-key pool with one retry-eligible failover step, the service normalizes keys once per start call, and the CLI path remains single-key. Round-robin order, failover semantics, no-distinct-key behavior, and the existing CLI-facing path are all covered by automated tests.

Milestone 4's automated verification is complete as well: the updated .NET and Python suites pass, the desktop app builds cleanly from the worktree, and a launch-level smoke test confirmed that the worktree executable starts and brings up a healthy backend. Milestone 4 review rounds 1 and 2 tightened the final sign-off boundary so the plan now consistently treats Settings and Live-page click-through as follow-up manual QA rather than a blocking requirement for this non-interactive terminal session, and the smoke-launched processes were explicitly cleaned up before a repeat build. The only remaining gap is that richer manual walkthrough, which should be handled as follow-up manual QA rather than an unresolved implementation defect.
...
1. `ui/` contains the Windows desktop app. The relevant files for this feature are:
   - `ui/GroqWhisper/Pages/SettingsPage.xaml` and `.cs`: multi-line API-key editor that can reveal, hide, clear, and report stored key counts while also saving non-secret backend settings.
   - `ui/GroqWhisper/ViewModels/LiveViewModel.cs`: start/stop view-model entry point that delegates multi-key request preparation before calling the backend.
   - `ui/GroqWhisper/Services/TranscriptionApiClient.cs`: HTTP client that submits the `POST /start` contract through the typed `api_keys` request body.
   - `ui/GroqWhisper.Core/WindowsSecretStore.cs`: encrypts and stores a versioned multi-key secret envelope under LocalApplicationData.
   ...
   - `ui/GroqWhisper.Tests/WindowsSecretStoreTests.cs`, `TranscriptionStartPreparationTests.cs`, and `TranscriptionStartRequestTests.cs`: .NET coverage for secret normalization/recovery, start-request preparation, and payload serialization.

2. `backend/` contains the FastAPI service and rolling transcription logic. The relevant files are:
   - `backend/src/groq_whisper_service/api.py`: translates HTTP JSON bodies into service start/stop/config calls, accepts `api_keys` on `/start`, and rejects secret fields in `/settings`.
   - `backend/src/groq_whisper_service/service.py`: state machine that normalizes `api_keys` once per start, runs preflight, creates the pool-backed client surface, and drives each rolling transcription request.
   ...
   - `backend/tests/test_service.py`: Python coverage for service transitions, API endpoint behavior, `/settings` secret rejection, and service-path pool wiring.
   - `backend/tests/test_client_pool.py`: direct Python coverage for normalization, round-robin ordering, failover, and no-distinct-key behavior.
   - `backend/tests/test_stable_prefix.py`: regression coverage for the untouched CLI-facing rolling-transcriber path.
...
After this milestone, the user can paste multiple keys into Settings, save them locally, clear them, and reveal them back into the editor. The UI clearly indicates that one key is expected per line and how many keys are currently stored. Verification in this terminal session is via `WindowsSecretStore` unit tests and solution build coverage; richer Settings-page click-through is tracked separately as follow-up manual QA. Completion requires the old single-key editor code paths to be removed or updated so the UI and storage semantics are internally consistent, requires an explicit unsupported-old-format message for legacy single-key local payloads, requires the new persisted plaintext to use a versioned envelope that can distinguish a new one-key save from the old raw-string payload, and requires the empty-editor Save path to leave stored keys unchanged while still permitting non-secret settings to be saved.

### Milestone 2: Frontend-to-backend contract moves to `api_keys`

After this milestone, clicking Start on the Live page sends a list of keys rather than a single key string. The backend `/start` endpoint validates the new list payload and rejects empty or malformed lists with actionable errors, while `/settings` continues rejecting all secret fields including `api_keys`. Verification in this terminal session is via backend API tests, frontend-side payload-shape and request-preparation tests, and later launch-level smoke coverage in Milestone 4; richer Live-page interaction remains follow-up manual QA. Completion requires all frontend start paths in this repo to use the new contract consistently.
